import "./PageToutesLesExperiences.css";
const PageToutesLesExperiences = () => {
  return (
    <div className="page-toutes-les-experiences">
      <div className="divchakra-stack195">
        <div className="divcss-1y21ojc5">
          <div className="divcss-jkl58t5">
            <div className="divchakra-stack196">
              <div className="divchakra-stack197">
                <div className="button-filter">
                  <div className="svg77">
                    <div className="frame77">
                      <img
                        className="vector-icon80"
                        alt=""
                        src="/vector30.svg"
                      />
                    </div>
                  </div>
                </div>
                <div className="divcss-1si4p9s">
                  <div className="form">
                    <div className="svg78">
                      <div className="frame78">
                        <img
                          className="vector-icon81"
                          alt=""
                          src="/vector31.svg"
                        />
                      </div>
                    </div>
                    <div className="input">
                      <b className="search-quests">Search Quests</b>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="divchakra-stack198">
              <div className="button40">
                <div className="divcss-1si4p9s">
                  <div className="pchakra-text159">
                    <b className="newest">newest</b>
                  </div>
                </div>
                <div className="spanchakra-button-icon35">
                  <div className="svg77">
                    <div className="frame79">
                      <img
                        className="vector-icon82"
                        alt=""
                        src="/vector32.svg"
                      />
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="divchakra-stack199">
            <div className="divcss-e03czl5">
              <div className="divchakra-stack200">
                <div />
                <div className="pchakra-text160">
                  <div className="all-experiences">{`ALL Experiences `}</div>
                </div>
              </div>
            </div>
          </div>
          <div className="divcss-1s1c1bz">
            <div className="divchakra-stack201">
              <div className="divchakra-stack202">
                <div className="divcss-1kq5yp1">
                  <div className="span52">
                    <img
                      className="uvbv6ka5rexhgdyvy9ccq4b5hpsf1y-icon"
                      alt=""
                      src="/uvbv6ka5rexhgdyvy9ccq4b5hpsf1ykhjwje4x2wdia@2x.png"
                    />
                  </div>
                </div>
                <div className="divchakra-stack203">
                  <div className="divchakra-stack204">
                    <div className="divcss-827vyq">
                      <div className="svg80">
                        <img className="frame-icon" alt="" src="/frame.svg" />
                      </div>
                    </div>
                    <div className="pchakra-text161">
                      <div className="dyor-find">{`DYOR & Find crypto data`}</div>
                    </div>
                  </div>
                  <div className="divchakra-stack205">
                    <div className="divcss-qm0npo">
                      <div className="span53">
                        <div className="image44">
                          <div className="image45" />
                        </div>
                        <div className="mirror938c2527png" />
                      </div>
                    </div>
                    <div className="pchakra-text162">
                      <div className="beginners">Beginners</div>
                    </div>
                  </div>
                  <div className="divchakra-stack206">
                    <div className="divchakra-stack207">
                      <div className="divcss-go2wv12">
                        <div className="svg81">
                          <div className="frame80">
                            <img
                              className="vector-icon83"
                              alt=""
                              src="/vector33.svg"
                            />
                          </div>
                        </div>
                      </div>
                      <div className="pchakra-text163">
                        <div className="beginners">500 APE</div>
                      </div>
                    </div>
                    <div className="divcss-dga45d5">
                      <div className="span54">
                        <div className="image46">
                          <div className="image47" />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="divchakra-stack208">
                <div className="divchakra-stack209">
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn25">
                      <div className="span55">
                        <img
                          className="meytabeth-icon"
                          alt=""
                          src="/meytabeth@2x.png"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn26">
                      <div className="span56">
                        <div className="image48">
                          <div className="image45" />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn27">
                      <div className="span57">
                        <div className="image44">
                          <div className="image45" />
                        </div>
                        <div className="dottyb240c024svg">
                          <div className="dottyb240c024svg1" />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="pchakra-text164">
                  <div className="claim-ongoing">3 Claim Ongoing</div>
                </div>
              </div>
            </div>
            <div className="divchakra-stack210">
              <div className="divchakra-stack202">
                <div className="divcss-1kq5yp1">
                  <div className="span52">
                    <img
                      className="i3dorcsj-uj2aymygvyeljpkyos-vb-icon"
                      alt=""
                      src="/8i3dorcsjuj2aymygvyeljpkyos-vbcr1zc2vcpe2i@2x.png"
                    />
                  </div>
                  <div className="divchakra-stack212">
                    <div className="beginners">New</div>
                  </div>
                </div>
                <div className="divchakra-stack213">
                  <div className="divchakra-stack204">
                    <div className="divcss-827vyq">
                      <div className="svg80">
                        <img className="frame-icon" alt="" src="/frame1.svg" />
                      </div>
                    </div>
                    <div className="pchakra-text165">
                      <div className="dyor-find">Create your first NFTs</div>
                    </div>
                  </div>
                  <div className="divchakra-stack215">
                    <div className="divcss-qm0npo">
                      <div className="span53">
                        <div className="image44">
                          <div className="image45" />
                        </div>
                        <div className="mirror938c2527png" />
                      </div>
                    </div>
                    <div className="pchakra-text166">
                      <div className="beginners">Beginners</div>
                    </div>
                  </div>
                  <div className="divchakra-stack216">
                    <div className="divchakra-stack207">
                      <div className="divcss-go2wv12">
                        <div className="svg81">
                          <div className="frame80">
                            <img
                              className="vector-icon83"
                              alt=""
                              src="/vector34.svg"
                            />
                          </div>
                        </div>
                      </div>
                      <div className="pchakra-text163">
                        <div className="beginners">500$ USDT</div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="divchakra-stack218">
                <div className="divchakra-stack209">
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn25">
                      <div className="span55">
                        <img
                          className="meytabeth-icon"
                          alt=""
                          src="/haoxiaoeth2@2x.png"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn29" />
                  </div>
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn30">
                      <div className="span56">
                        <div className="image48">
                          <div className="image45" />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="pchakra-text168">
                  <div className="claim-ongoing">1 claim ongoing</div>
                </div>
              </div>
            </div>
            <div className="divchakra-stack220">
              <div className="divchakra-stack221" />
            </div>
            <div className="divchakra-stack222" />
          </div>
          <div className="divcss-1s1c1bz1">
            <div className="divchakra-stack223">
              <div className="divchakra-stack202">
                <div className="divcss-1kq5yp1">
                  <div className="span52">
                    <img
                      className="uvbv6ka5rexhgdyvy9ccq4b5hpsf1y-icon"
                      alt=""
                      src="/uvbv6ka5rexhgdyvy9ccq4b5hpsf1ykhjwje4x2wdia1@2x.png"
                    />
                  </div>
                  <div className="divchakra-stack225">
                    <div className="trendy">Trendy</div>
                  </div>
                </div>
                <div className="divchakra-stack213">
                  <div className="divchakra-stack204">
                    <div className="divcss-827vyq">
                      <div className="svg80">
                        <img className="frame-icon" alt="" src="/frame.svg" />
                      </div>
                    </div>
                    <div className="pchakra-text161">
                      <div className="faire-un-swap">Faire un swap</div>
                    </div>
                  </div>
                  <div className="divchakra-stack205">
                    <div className="divcss-qm0npo">
                      <div className="span53">
                        <div className="image44">
                          <div className="image45" />
                        </div>
                        <div className="mirror938c2527png2" />
                      </div>
                    </div>
                    <div className="pchakra-text162">
                      <div className="trendy">Medium</div>
                    </div>
                  </div>
                  <div className="divchakra-stack206">
                    <div className="divchakra-stack207">
                      <div className="divcss-go2wv12">
                        <div className="svg81">
                          <div className="frame80">
                            <img
                              className="vector-icon83"
                              alt=""
                              src="/vector33.svg"
                            />
                          </div>
                        </div>
                      </div>
                      <div className="pchakra-text163">
                        <div className="trendy">1000$ INCH</div>
                      </div>
                    </div>
                    <div className="divcss-dga45d5">
                      <div className="span54">
                        <div className="image46">
                          <div className="image47" />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="divchakra-stack231">
                <div className="divchakra-stack209">
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn25">
                      <div className="span55">
                        <img
                          className="meytabeth-icon"
                          alt=""
                          src="/meytabeth@2x.png"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn32">
                      <div className="span56">
                        <div className="image48">
                          <div className="image45" />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn33">
                      <div className="span57">
                        <div className="image44">
                          <div className="image45" />
                        </div>
                        <div className="dottyb240c024svg">
                          <div className="dottyb240c024svg1" />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="pchakra-text164">
                  <div className="claim-ongoing2">12 Claim Ongoing</div>
                </div>
              </div>
            </div>
            <div className="divchakra-stack233">
              <div className="divchakra-stack202">
                <div className="divcss-1kq5yp1">
                  <div className="span52">
                    <img
                      className="i3dorcsj-uj2aymygvyeljpkyos-vb-icon"
                      alt=""
                      src="/8i3dorcsjuj2aymygvyeljpkyos-vbcr1zc2vcpe2i1@2x.png"
                    />
                  </div>
                  <div className="divchakra-stack212">
                    <div className="trendy">New</div>
                  </div>
                </div>
                <div className="divchakra-stack213">
                  <div className="divchakra-stack204">
                    <div className="divcss-827vyq">
                      <div className="svg80">
                        <img className="frame-icon" alt="" src="/frame1.svg" />
                      </div>
                    </div>
                    <div className="pchakra-text165">
                      <div className="faire-un-swap">Create ENS</div>
                    </div>
                  </div>
                  <div className="divchakra-stack215">
                    <div className="divcss-qm0npo">
                      <div className="span53">
                        <div className="image44">
                          <div className="image45" />
                        </div>
                        <div className="mirror938c2527png" />
                      </div>
                    </div>
                    <div className="pchakra-text166">
                      <div className="trendy">Beginners</div>
                    </div>
                  </div>
                  <div className="divchakra-stack216">
                    <div className="divchakra-stack207">
                      <div className="divcss-go2wv12">
                        <div className="svg81">
                          <div className="frame80">
                            <img
                              className="vector-icon83"
                              alt=""
                              src="/vector34.svg"
                            />
                          </div>
                        </div>
                      </div>
                      <div className="pchakra-text163">
                        <div className="trendy">1000$ ETH</div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="divchakra-stack218">
                <div className="divchakra-stack209">
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn25">
                      <div className="span55">
                        <img
                          className="meytabeth-icon"
                          alt=""
                          src="/haoxiaoeth2@2x.png"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn29" />
                  </div>
                  <div className="spanchakra-button-icon35">
                    <div className="divcss-1y1nkkn30">
                      <div className="span56">
                        <div className="image48">
                          <div className="image45" />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="pchakra-text168">
                  <div className="claim-ongoing2">1 claim ongoing</div>
                </div>
              </div>
            </div>
            <div className="divchakra-stack243">
              <div className="divchakra-stack221" />
            </div>
            <div className="divchakra-stack245" />
          </div>
          <div className="divcss-1s1c1bz2">
            <div className="divchakra-stack245" />
            <div className="divchakra-stack243">
              <div className="divchakra-stack221" />
            </div>
            <div className="divchakra-stack245" />
          </div>
          <div className="divcss-y0pad5">
            <div className="divchakra-stack250">
              <div className="divchakra-stack251">
                <div className="button41">
                  <div className="spanchakra-button-icon36">
                    <div className="svg77">
                      <div className="frame84">
                        <img
                          className="vector-icon87"
                          alt=""
                          src="/vector35.svg"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="divcss-e03czl6">
                    <div className="pchakra-text177">
                      <div className="faire-un-swap">Notifications</div>
                    </div>
                    <div className="divchakra-stack252">
                      <div className="divcss-go2wv16">
                        <div className="divcss-1ldmaet5" />
                      </div>
                      <div className="pchakra-text178">
                        <div className="trendy">1</div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="button42">
                <div className="spanchakra-button-icon35">
                  <div className="svg77">
                    <div className="frame85">
                      <img
                        className="vector-icon88"
                        alt=""
                        src="/vector36.svg"
                      />
                    </div>
                  </div>
                </div>
                <div className="pchakra-text179">
                  <a href="/">
                    <div className="home5">Home</div>
                  </a>
                </div>
              </div>
              <div className="divchakra-stack253">
                <div className="button43">
                  <div className="spanchakra-button-icon35">
                    <div className="svg77">
                      <div className="frame86">
                        <img
                          className="vector-icon89"
                          alt=""
                          src="/vector10.svg"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="pchakra-text180">
                    <div className="level5">Level</div>
                  </div>
                </div>
                <div className="button44">
                  <div className="spanchakra-button-icon35">
                    <div className="svg77">
                      <div className="frame87">
                        <img
                          className="vector-icon90"
                          alt=""
                          src="/vector28.svg"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="pchakra-text181">
                    <div className="experience5">Experience</div>
                  </div>
                </div>
              </div>
              <div className="divchakra-stack254">
                <div className="pchakra-text182">
                  <div className="trendy">Account</div>
                </div>
                <div className="button45">
                  <div className="spanchakra-button-icon35">
                    <div className="svg77">
                      <div className="frame88">
                        <img
                          className="vector-icon87"
                          alt=""
                          src="/vector23.svg"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="pchakra-text183">
                    <div className="account11">Account</div>
                  </div>
                </div>
              </div>
              <div className="divchakra-stack254">
                <div className="pchakra-text184">
                  <div className="trendy">Resources</div>
                </div>
                <div className="button46">
                  <div className="spanchakra-button-icon35">
                    <div className="svg77">
                      <div className="frame89">
                        <img
                          className="vector-icon92"
                          alt=""
                          src="/vector13.svg"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="pchakra-text185">
                    <div className="links5">Links</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default PageToutesLesExperiences;
