import "./PageExperienceN3.css";
const PageExperienceN3 = () => {
  return (
    <div className="page-experience-n3">
      <div className="divcss-1y21ojc1">
        <div className="divcss-sdkmul1">
          <div className="divchakra-stack35">
            <a href="/">
              <img className="divchakra-stack-item" alt="" src="/ellipse-1@2x.png" />
            </a>
            <div className="divcss-1uod83n1" />
          </div>
          <div className="divchakra-stack36">
            <div className="button-help1">
              <div className="svg14">
                <div className="frame14">
                  <img className="vector-icon14" alt="" src="/vector14.svg" />
                </div>
              </div>
            </div>
            <div className="divcss-06">
              <div className="button8">
                <div className="spanchakra-button-icon7">
                  <div className="span10">
                    <div className="image8">
                      <div className="image9" />
                    </div>
                    <div className="wallet2f170466svg1">
                      <img className="wallet2f170466svg-icon1" alt="" src="/wallet2f170466svg1.svg" />
                    </div>
                  </div>
                </div>
                <div className="pchakra-text30">
                  <div className="id-010271">ID : 01027</div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="divcss-fk4jif1">
          <div className="divchakra-stack37">
            <div className="divchakra-stack38">
              <div className="divchakra-stack39">
                <div className="divchakra-stack40">
                  <div className="span11">
                    <div className="image10">
                      <div className="image11" />
                    </div>
                    <img className="mirror938c2527png-icon1" alt="" src="/mirror938c2527png@2x.png" />
                  </div>
                  <div className="pchakra-text31">
                    <div className="inch-swap1">{`1 Inch / Swap yours Tokens  `}</div>
                  </div>
                </div>
                <div className="pchakra-text32">
                  <b className="swap-dai-to1">Swap DAI to USDT</b>
                </div>
              </div>
              <div className="divchakra-stack41">
                <div className="divchakra-stack42">
                  <div className="divchakra-stack43">
                    <div className="divcss-go2wv2">
                      <div className="svg15">
                        <div className="frame15">
                          <img className="vector-icon15" alt="" src="/vector1.svg" />
                        </div>
                      </div>
                    </div>
                    <div className="pchakra-text33">
                      <div className="sign-transaction-with">{`1000$ 1INCH `}</div>
                    </div>
                  </div>
                  <div className="divcss-dga45d1">
                    <div className="span12">
                      <div className="image10">
                        <div className="image11" />
                      </div>
                      <div className="optimism202c83a8png1" />
                    </div>
                  </div>
                </div>
                <div className="divchakra-stack44">
                  <div className="divchakra-stack45">
                    <div className="svg15">
                      <div className="frame16">
                        <img className="vector-icon16" alt="" src="/vector15.svg" />
                      </div>
                    </div>
                    <div className="pchakra-text34">
                      <div className="sign-transaction-with">Challenge</div>
                    </div>
                  </div>
                  <div className="pchakra-text35">
                    <div className="swap-token-a1">Swap token A to token B using DeFi protocol</div>
                  </div>
                </div>
                <div className="divchakra-stack46">
                  <div className="divchakra-stack47">
                    <div className="svg15">
                      <div className="frame17">
                        <img className="vector-icon17" alt="" src="/vector3.svg" />
                      </div>
                    </div>
                    <div className="pchakra-text36">
                      <div className="sign-transaction-with">Difficulté</div>
                    </div>
                  </div>
                  <div className="pchakra-text37">
                    <div className="facile1">Facile</div>
                  </div>
                </div>
                <div className="divchakra-stack48">
                  <div className="divchakra-stack49">
                    <div className="svg15">
                      <div className="frame18">
                        <img className="vector-icon18" alt="" src="/vector16.svg" />
                      </div>
                    </div>
                    <div className="pchakra-text38">
                      <div className="sign-transaction-with">Claimed</div>
                    </div>
                  </div>
                  <div className="pchakra-text39">
                    <div className="swap-token-a1">1000$ en 1INCH token to share</div>
                  </div>
                </div>
              </div>
            </div>
            <div className="divchakra-stack50">
              <div className="divchakra-stack51">
                <div className="svg19">
                  <div className="frame19">
                    <img className="vector-icon19" alt="" src="/vector17.svg" />
                  </div>
                </div>
                <div className="pchakra-text40">
                  <div className="requirements">Requirements</div>
                </div>
              </div>
              <div className="divchakra-stack52">
                <div className="divcss-p9xbm1">
                  <div className="divcss-rlhbmu">
                    <div className="button-checkbox" />
                  </div>
                  <div className="divchakra-stack53">
                    <div className="pchakra-text41">
                      <div className="read-what-is">Read what is 1 inch and what’s is for</div>
                    </div>
                    <div className="divchakra-stack54">
                      <div className="divchakra-stack55">
                        <div className="divcss-go2wv2">
                          <div className="span13">
                            <div className="image14">
                              <div className="image15" />
                            </div>
                            <img className="optimism202c83a8png-icon" alt="" src="/optimism202c83a8png@2x.png" />
                          </div>
                        </div>
                        <div className="pchakra-text33">
                          <div className="sign-transaction-with">
                            Sign transaction with our app (wallet is integrate)
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="divchakra-stack56">
              <div className="divchakra-stack57">
                <div className="svg19">
                  <div className="frame20">
                    <img className="vector-icon20" alt="" src="/vector18.svg" />
                  </div>
                </div>
                <div className="pchakra-text43">
                  <div className="requirements">Related Links</div>
                </div>
              </div>
              <div className="divchakra-stack52">
                <div className="divcss-p9xbm11">
                  <div className="divcss-a20ioa">
                    <div className="svg15">
                      <div className="frame21">
                        <img className="vector-icon21" alt="" src="/vector19.svg" />
                      </div>
                    </div>
                  </div>
                  <div className="link2" />
                  <div className="https1inchio">https://1inch.io/</div>
                </div>
              </div>
            </div>
          </div>
          <div className="divchakra-stack59">
            <div className="divchakra-stack60">
              <div className="svg22">
                <div className="frame22" />
              </div>
              <img className="vector-icon22" alt="" src="/vector20.svg" />
              <div className="description2">
                <p className="this-experience-involves">Description</p>
              </div>
              <div className="pchakra-text44" />
            </div>
            <div className="pchakra-text45">
              <img className="pchakra-text-item" alt="" src="/rectangle-6@2x.png" />
              <div className="this-experience-involves-container">
                <span className="this-experience-involves-container1">
                  <p className="this-experience-involves">
                    This experience involves a token swap using Gnosis Chain and the 1inch aggregator. The user will
                    exchange 50 DAI for USDT. Gnosis Chain provides a secure environment for the transaction, while the
                    1inch aggregator ensures the best available exchange rate across multiple decentralized exchanges.
                    During the process, the user's transaction is monitored and verified for accuracy.
                  </p>
                </span>
              </div>
            </div>
          </div>
          <div className="divchakra-stack61">
            <div className="divchakra-stack62">
              <div className="pchakra-text46">
                <div className="examples-of-nouns1">Examples of nouns learners</div>
              </div>
              <div className="divchakra-stack63">
                <div className="svg15">
                  <div className="frame23">
                    <img className="vector-icon23" alt="" src="/vector5.svg" />
                  </div>
                </div>
                <div className="pchakra-text47">
                  <div className="claim-issue-report1">Claim Issue Report</div>
                </div>
              </div>
              <div className="divcss-1u95mlq1">
                <div className="span14">
                  <img
                    className="j7jl0vs06s3deq80ehhnelx1rq2faz-icon1"
                    alt=""
                    src="/j7jl0vs06s3deq80ehhnelx1rq2faztdzzb0na7qahk@2x.png"
                  />
                  <div className="capture-decran-2023-07-23-a1" />
                </div>
              </div>
            </div>
            <div className="divchakra-stack64">
              <div className="divchakra-stack65">
                <div className="pchakra-text48">
                  <div className="requirements">Achieved Accounts</div>
                </div>
                <div className="divcss-1ee2lil1" />
                <div className="span15">
                  <div className="image10">
                    <div className="image11" />
                  </div>
                  <div className="dotty-gray418199d2svg2">
                    <div className="dotty-gray418199d2svg3">
                      <img className="vector-icon24" alt="" src="/vector6.svg" />
                    </div>
                  </div>
                </div>
              </div>
              <div className="divchakra-stack66">
                <div className="divchakra-stack67">
                  <div className="divcss-07">
                    <div className="divcss-1y1nkkn5">
                      <div className="span14">
                        <img className="haoxiaoeth-icon1" alt="" src="/haoxiaoeth@2x.png" />
                      </div>
                    </div>
                  </div>
                  <div className="pchakra-text49">
                    <div className="examples-of-nouns1">haoxiao.eth</div>
                  </div>
                </div>
                <div className="divchakra-stack67">
                  <div className="divcss-07">
                    <div className="divcss-1y1nkkn5">
                      <div className="span14">
                        <img className="haoxiaoeth-icon1" alt="" src="/mo115eth@2x.png" />
                      </div>
                    </div>
                  </div>
                  <div className="pchakra-text50">
                    <div className="examples-of-nouns1">mo115.eth</div>
                  </div>
                </div>
                <div className="divchakra-stack67">
                  <div className="divcss-07">
                    <div className="divcss-1y1nkkn5">
                      <div className="span14">
                        <img className="haoxiaoeth-icon1" alt="" src="/neparaeth@2x.png" />
                      </div>
                    </div>
                  </div>
                  <div className="pchakra-text51">
                    <div className="examples-of-nouns1">nepara.eth</div>
                  </div>
                </div>
                <div className="divchakra-stack67">
                  <div className="divcss-07">
                    <div className="divcss-1y1nkkn5">
                      <div className="span14">
                        <img className="haoxiaoeth-icon1" alt="" src="/0x68159eth@2x.png" />
                      </div>
                    </div>
                  </div>
                  <div className="pchakra-text52">
                    <div className="examples-of-nouns1">0x68159.eth</div>
                  </div>
                </div>
                <div className="divchakra-stack67">
                  <div className="divcss-07">
                    <div className="divcss-1y1nkkn5">
                      <div className="span14">
                        <img className="haoxiaoeth-icon1" alt="" src="/truete11ereth@2x.png" />
                      </div>
                    </div>
                  </div>
                  <div className="pchakra-text53">
                    <div className="examples-of-nouns1">truete11er.eth</div>
                  </div>
                </div>
              </div>
            </div>
            <div className="divframer-gvo3gs-container1">
              <a href="/1inch-experience">
                <div className="link3">
                  <b className="get-started">Get Started</b>
                </div>
              </a>
            </div>
          </div>
        </div>
        <div className="divcss-jkl58t1">
          <div className="divchakra-stack72">
            <a href="/launchapp">
              <div className="pchakra-text54">
                <b className="back-to-all1">Back to all experiences</b>
              </div>
            </a>
            <div className="button9">
              <div className="svg19">
                <div className="frame24">
                  <img className="vector-icon25" alt="" src="/vector7.svg" />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="divcss-y0pad1">
        <div className="divchakra-stack73">
          <div className="divchakra-stack74">
            <div className="button10">
              <div className="spanchakra-button-icon8">
                <div className="svg19">
                  <div className="frame25">
                    <img className="vector-icon26" alt="" src="/vector21.svg" />
                  </div>
                </div>
              </div>
              <div className="divcss-e03czl1">
                <div className="pchakra-text55">
                  <div className="examples-of-nouns1">Notifications</div>
                </div>
                <div className="divchakra-stack75">
                  <div className="divcss-go2wv4">
                    <div className="divcss-1ldmaet1" />
                  </div>
                  <div className="pchakra-text56">
                    <div className="sign-transaction-with">1</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="button11">
            <div className="divcss-06">
              <div className="svg19">
                <div className="frame26">
                  <img className="vector-icon27" alt="" src="/vector22.svg" />
                </div>
              </div>
            </div>
            <div className="pchakra-text57">
              <div className="home1">
                <a href="/launchapp">Home</a>
              </div>
            </div>
          </div>
          <div className="divchakra-stack76">
            <div className="button12">
              <div className="divcss-06">
                <div className="svg19">
                  <div className="frame27">
                    <img className="vector-icon28" alt="" src="/vector10.svg" />
                  </div>
                </div>
              </div>
              <div className="pchakra-text58">
                <div className="level1">
                  <a href="/levels">Level</a>
                </div>
              </div>
            </div>
            <div className="button13">
              <div className="divcss-06">
                <div className="svg19">
                  <div className="frame28">
                    <img className="vector-icon29" alt="" src="/vector11.svg" />
                  </div>
                </div>
              </div>
              <div className="pchakra-text59">
                <div className="experience1">
                  <a href="/experiences">Experience</a>
                </div>
              </div>
            </div>
          </div>
          <div className="divchakra-stack77">
            <div className="pchakra-text60">
              <div className="sign-transaction-with">Account</div>
            </div>
            <div className="button14">
              <div className="divcss-06">
                <div className="svg19">
                  <div className="frame29">
                    <img className="vector-icon26" alt="" src="/vector23.svg" />
                  </div>
                </div>
              </div>
              <div className="pchakra-text61">
                <div className="account3">Account</div>
              </div>
            </div>
          </div>
          <div className="divchakra-stack77">
            <div className="pchakra-text62">
              <div className="sign-transaction-with">Resources</div>
            </div>
            <div className="button15">
              <div className="divcss-06">
                <div className="svg19">
                  <div className="frame20">
                    <img className="vector-icon20" alt="" src="/vector13.svg" />
                  </div>
                </div>
              </div>
              <div className="pchakra-text63">
                <div className="links1">Links</div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default PageExperienceN3;
