import "./HomePage.css";
const HomePage = () => {
  return (
    <div className="site-web">
      <div className="tout-le-site-tout-le-long">
        <div className="panneau-1">
          <img className="image-icon" alt="" src="/image@2x.png" />
          <div className="introduction">
            <div className="link10">
              <div className="divframer-xhvdkm">
                <b className="new2">NEW</b>
              </div>
              <div className="divframer-7ncycj">
                <div className="defi-experience-available">DeFi experience available</div>
              </div>
            </div>
            <div className="divframer-1u3vmnn">
              <div className="divframer-1d9jrl6">
                <div className="divframer-ygd3m7">
                  <div className="divframer-19xbngw">
                    <div className="discover">Discover</div>
                  </div>
                  <div className="divframer-1tslim6">
                    <div className="discover">{`a New Way of `}</div>
                  </div>
                </div>
                <div className="divframer-ik1l8j-container">
                  <div className="learning-and-engaging">Learning and Engaging!</div>
                </div>
              </div>
              <div className="divframer-11g15lb">
                <div className="pframer-text">
                  <div className="embark-on-your">Embark on your educational journey now!</div>
                </div>
              </div>
            </div>
            <div className="divframer-1tguap6">
              <div className="divframer-gvo3gs-container5">
                <a href="/launchapp">
                  <div className="link11">
                    <b className="get-started3">Get Started</b>
                  </div>
                </a>
              </div>
              <div className="divframer-ivci4s-container">
                <div className="link12">
                  <div className="divframer-1bj0y4t">
                    <div className="divframer-g8h6s6-container">
                      <div className="svg122">
                        <img className="frame-icon4" alt="" src="/frame2.svg" />
                      </div>
                    </div>
                    <div className="divframer-1tta95z">
                      <div className="l3arn">@l3arn</div>
                    </div>
                  </div>
                  <div className="divframer-16typo1-container">
                    <div className="svg122">
                      <img className="frame-icon5" alt="" src="/frame3.svg" />
                    </div>
                  </div>
                  <div className="pseudo" />
                </div>
              </div>
            </div>
          </div>
          <div className="diviseur-petit-ligne-blanche" />
          <div className="nav">
            <div className="barre-de-nav">
              <div className="divframer-1cvrqom">
                <img className="divframer-1cvrqom-child" alt="" src="/ellipse-11@2x.png" />
              </div>
              <div className="divframer-taxvkj-container">
                <div className="divframer-h314k6">
                  <div className="divframer-px3zsv-container">
                    <div className="link13">
                      <div className="span85">
                        <div className="team">Home</div>
                      </div>
                      <div className="pseudo1" />
                    </div>
                  </div>
                  <div className="divframer-11pliaz-container">
                    <div className="link14">
                      <div className="divframer-icxdue">
                        <div className="divframer-mizgsx">
                          <div className="span86">
                            <div className="team">Team</div>
                          </div>
                        </div>
                        <div className="divframer-154idwg-container">
                          <div className="svg122">
                            <img className="frame-icon6" alt="" src="/frame4.svg" />
                          </div>
                        </div>
                      </div>
                      <div className="pseudo1" />
                    </div>
                  </div>
                  <div className="divframer-hyfadc-container">
                    <div className="link14">
                      <div className="divframer-icxdue">
                        <div className="divframer-mizgsx">
                          <div className="span87">
                            <div className="team">Partnership</div>
                          </div>
                        </div>
                        <div className="divframer-154idwg-container">
                          <div className="svg122">
                            <img className="frame-icon6" alt="" src="/frame5.svg" />
                          </div>
                        </div>
                      </div>
                      <div className="pseudo1" />
                    </div>
                  </div>
                  <div className="divframer-u6qzx1-container">
                    <div className="link14">
                      <div className="divframer-icxdue">
                        <div className="divframer-mizgsx">
                          <div className="span88">
                            <div className="team">Job</div>
                          </div>
                        </div>
                        <div className="divframer-154idwg-container">
                          <div className="svg122">
                            <img className="frame-icon6" alt="" src="/frame6.svg" />
                          </div>
                        </div>
                      </div>
                      <div className="pseudo1" />
                    </div>
                  </div>
                  <div className="divframer-193fho5-container">
                    <div className="link14">
                      <div className="divframer-icxdue">
                        <div className="divframer-mizgsx">
                          <div className="span89">
                            <div className="team">Blog</div>
                          </div>
                        </div>
                        <div className="divframer-154idwg-container">
                          <div className="svg122">
                            <img className="frame-icon6" alt="" src="/frame7.svg" />
                          </div>
                        </div>
                      </div>
                      <div className="pseudo1" />
                    </div>
                  </div>
                  <div className="pseudo6" />
                </div>
              </div>
              <div className="divframer-7pmrb3">
                <a href="/launchapp">
                  <div className="link18">
                    <b className="get-started3">Get Started</b>
                  </div>
                </a>
              </div>
            </div>
          </div>
        </div>
        <div className="panneau-2-unlock-your-poten">
          <div className="divframer-fcp6mx">
            <div className="divframer-13p91mx-container">
              <div className="divframer-15bkj6r">
                <div className="divframer-29orjw" />
                <div className="divframer-29orjw" />
                <div className="divframer-29orjw" />
              </div>
            </div>
            <div className="divframer-63e09b">
              <div className="discover">Unlock Your Potential</div>
            </div>
            <div className="divframer-rqmtfk-container">
              <div className="divframer-15bkj6r">
                <div className="divframer-29orjw1" />
                <div className="divframer-29orjw1" />
                <div className="divframer-29orjw1" />
              </div>
            </div>
          </div>
          <div className="divframer-a2zod0">
            <div className="header">
              <div className="divframer-1h7ubfw">
                <div className="divframer-nw1ujj">
                  <div className="divframer-1w6twvx">
                    <div className="divframer-1bpwmf1" />
                    <b className="heading-2"> Learn through Play</b>
                  </div>
                  <div className="divframer-6kc6is">
                    <div className="heading-21">
                      <div className="experience-the-power">
                        Experience the power of gamification in education. Our platform offers interactive and engaging
                        learning experiences that make education fun and effective.
                      </div>
                    </div>
                  </div>
                </div>
                <div className="divframer-3wws4p-container">
                  <div className="link19">
                    <a href="/launchapp">
                      <div className="divframer-5apapq" />
                      <b className="get-started5">{`Get Started `}</b>
                    </a>
                    <div className="divframer-5wpwc2-container">
                      <div className="svg122">
                        <img className="frame-icon6" alt="" src="/frame8.svg" />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="divframer-1wuwanx">
                <div className="divframer-s9ql23">
                  <div className="divframer-5rrw4d">
                    <div className="divframer-1labwdg">
                      <div className="divframer-1ks9sk">
                        <img
                          className="ysm8nmktsf750u1a0at8z41qpng-icon"
                          alt=""
                          src="/8ysm8nmktsf750u1a0at8z41qpng@2x.png"
                        />
                      </div>
                      <div className="divframer-1e7ywhx">
                        <div className="divframer-s11gfa">
                          <div className="heading-3">Playful Experiences</div>
                        </div>
                        <div className="divframer-qg1gs0">
                          <div className="pframer-text1">
                            <div className="engage-in-interactive-container">
                              <p className="engage-in-interactive">Engage in interactive activities and experiences</p>
                              <p className="engage-in-interactive">that will captivate your curiosity!</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="divframer-5rrw4d">
                    <div className="divframer-1labwdg">
                      <div className="divframer-1ks9sk">
                        <img
                          className="ysm8nmktsf750u1a0at8z41qpng-icon"
                          alt=""
                          src="/0ourk52cxf0erp8yejvujdvij4png@2x.png"
                        />
                      </div>
                      <div className="divframer-1se1xwx">
                        <div className="divframer-drbfyz">
                          <div className="heading-3">Level Based</div>
                        </div>
                        <div className="divframer-qg1gs0">
                          <div className="pframer-text2">
                            <div className="engage-in-interactive-container">
                              <p className="engage-in-interactive">{`Progress through levels, with each experience `}</p>
                              <p className="engage-in-interactive">designed to match expertise!</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="divframer-5rrw4d">
                    <div className="divframer-1labwdg">
                      <div className="divframer-1ks9sk">
                        <img
                          className="ysm8nmktsf750u1a0at8z41qpng-icon"
                          alt=""
                          src="/rujrnrvynhywvawzvb8dwgsdwpng@2x.png"
                        />
                      </div>
                      <div className="divframer-1e7ywhx">
                        <div className="divframer-1h5b43z">
                          <div className="heading-3">Earn Money, Rewards, and Goodies</div>
                        </div>
                        <div className="divframer-14588ts">
                          <div className="pframer-text3">
                            <div className="engage-in-interactive-container">
                              <p className="engage-in-interactive">{`Get rewarded for dedication and accomplishments! `}</p>
                              <p className="engage-in-interactive">{`Excel in educational pursuits to earn money, exciting `}</p>
                              <p className="engage-in-interactive">rewards, and goodies.</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="divframer-1ax1cz6">
                  <div className="divframer-1ys5o81">
                    <img
                      className="capture-decran-2023-07-22-a"
                      alt=""
                      src="/capture-decran-20230722-a-1348-1@2x.png"
                    />
                    <img
                      className="atkuwgqtjemaqstmakjtnfzlmqpng-icon"
                      alt=""
                      src="/atkuwgqtjemaqstmakjtnfzlmqpng@2x.png"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="panneau-3-how-to-play">
          <div className="divframer-1jmlo0p">
            <div className="divframer-1cnxjck-container">
              <div className="divframer-15bkj6r">
                <div className="divframer-29orjw2" />
                <div className="divframer-29orjw2" />
                <div className="divframer-29orjw2" />
              </div>
            </div>
            <div className="divframer-1l7woxr">
              <div className="discover">{`How to play `}</div>
            </div>
            <div className="divframer-1cnxjck-container">
              <div className="divframer-15bkj6r">
                <div className="divframer-29orjw2" />
                <div className="divframer-29orjw2" />
                <div className="divframer-29orjw2" />
              </div>
            </div>
          </div>
          <div className="divframer-1a7pbcy">
            <div className="divframer-c8el3u">
              <div className="divframer-zvfb7l">
                <img
                  className="lz3ut30tqkxsfuijogquvoywyepng-icon"
                  alt=""
                  src="/lz3ut30tqkxsfuijogquvoywyepng@2x.png"
                />
              </div>
              <div className="divframer-1hudja9">
                <div className="divframer-iq92uh">
                  <div className="create-your-account-container">
                    <span>{`Create Your `}</span>
                    <b className="steps">Account</b>
                  </div>
                </div>
                <div className="divframer-jics4j">
                  <div className="pframer-text4">
                    <div className="sign-up-and-container">
                      <span className="sign-up-and-container1">
                        <p className="engage-in-interactive">{`Sign up and create your account easily by `}</p>
                        <p className="engage-in-interactive">connecting with your preferred social network.</p>
                      </span>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="divframer-es7di">
              <div className="divframer-3xhhex">
                <img
                  className="lz3ut30tqkxsfuijogquvoywyepng-icon"
                  alt=""
                  src="/lz3ut30tqkxsfuijogquvoywyepng1@2x.png"
                />
              </div>
              <div className="divframer-1hudja9">
                <div className="divframer-koz047">
                  <div className="pframer-text5">
                    <div className="create-your-account-container">
                      <span>{`Choose Your `}</span>
                      <b className="experience8">Experience</b>
                      <span>{` `}</span>
                    </div>
                  </div>
                </div>
                <div className="divframer-bx5koi">
                  <div className="pframer-text6">
                    <div className="browse-our-selection-container">
                      <span className="sign-up-and-container1">
                        <p className="engage-in-interactive">&nbsp;</p>
                        <p className="engage-in-interactive">&nbsp;</p>
                        <p className="engage-in-interactive">
                          Browse our selection of diverse experiences, each carefully curated to match your expertise
                          and interests.
                        </p>
                      </span>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="divframer-139jdsh">
              <div className="divframer-3xhhex">
                <img
                  className="lz3ut30tqkxsfuijogquvoywyepng-icon"
                  alt=""
                  src="/lz3ut30tqkxsfuijogquvoywyepng2@2x.png"
                />
              </div>
              <div className="divframer-1l7i8oc">
                <div className="divframer-qfsz04">
                  <div className="pframer-text7">
                    <div className="create-your-account-container">
                      <span>{`Follow the `}</span>
                      <b className="steps">Steps</b>
                    </div>
                  </div>
                </div>
                <div className="divframer-1xcw8hp">
                  <div className="pframer-text8">
                    <div className="dive-into-your-container">
                      <span className="sign-up-and-container1">
                        <p className="engage-in-interactive">&nbsp;</p>
                        <p className="engage-in-interactive">&nbsp;</p>
                        <p className="engage-in-interactive">
                          Dive into your chosen experience and follow the step-by-step instructions provided.
                        </p>
                      </span>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="panneau-4-join-our-community">
          <div className="divframer-4w1knt">
            <div className="divframer-10xgs95-container">
              <div className="divframer-15bkj6r">
                <div className="divframer-29orjw4" />
                <div className="divframer-29orjw4" />
                <div className="divframer-29orjw4" />
              </div>
            </div>
            <div className="divframer-18g420e">
              <div className="discover">Join our community</div>
            </div>
            <div className="divframer-10xgs95-container">
              <div className="divframer-15bkj6r">
                <div className="divframer-29orjw4" />
                <div className="divframer-29orjw4" />
                <div className="divframer-29orjw4" />
              </div>
            </div>
          </div>
          <div className="divframer-ptd4hm">
            <div className="divframer-1ld1i3k">
              <div className="divframer-ttxvfo">
                <div className="divframer-a5tyne">
                  <div className="divframer-29fkbj">
                    <div className="divframer-z5tj6m">
                      <div className="image78">
                        <img className="vector-icon122" alt="" src="/vector44.svg" />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="divframer-1bix8f1">
                  <div className="create-your-account-container">Follow on twitter</div>
                </div>
                <div className="divframer-aswy28-container">
                  <div className="link20">
                    <div className="divframer-1jn56rz">
                      <div className="defi-experience-available">Follow @l3arn</div>
                    </div>
                    <div className="divframer-1g9tish-container">
                      <div className="svg122">
                        <img className="frame-icon6" alt="" src="/frame9.svg" />
                      </div>
                    </div>
                    <div className="pseudo7" />
                  </div>
                </div>
              </div>
              <div className="divframer-1okfsnf">
                <div className="divframer-1snlpco">
                  <div className="divframer-kl6att">
                    <div className="svg122">
                      <div className="frame118">
                        <img className="svg-2080539423-2943-icon" alt="" src="/svg2080539423-2943.svg" />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="divframer-c4g5nu">
                  <div className="create-your-account-container">Join our discord</div>
                </div>
                <div className="divframer-aswy28-container">
                  <div className="link20">
                    <div className="divframer-1jn56rz1">
                      <div className="defi-experience-available">Join Discord</div>
                    </div>
                    <div className="divframer-1g9tish-container">
                      <div className="svg122">
                        <img className="frame-icon6" alt="" src="/frame10.svg" />
                      </div>
                    </div>
                    <div className="pseudo7" />
                  </div>
                </div>
              </div>
              <div className="divframer-1okfsnf">
                <div className="divframer-1bafmeh">
                  <div className="svg122">
                    <div className="frame119">
                      <img className="svg2119534450-1294-icon" alt="" src="/svg2119534450-1294.svg" />
                    </div>
                  </div>
                </div>
                <div className="divframer-1be4hhv">
                  <div className="create-your-account-container">Join our github</div>
                </div>
                <div className="divframer-aswy28-container">
                  <div className="link20">
                    <div className="divframer-1jn56rz2">
                      <div className="defi-experience-available">Open Repository</div>
                    </div>
                    <div className="divframer-1g9tish-container">
                      <div className="svg122">
                        <img className="frame-icon6" alt="" src="/frame11.svg" />
                      </div>
                    </div>
                    <div className="pseudo7" />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="footer">
          <div className="divframer-19w2hbr">
            <div className="divframer-p5ody2">
              <div className="divframer-sig6s6">
                <div className="divframer-1jgbggs">
                  <div className="heading-6">General</div>
                </div>
                <div className="divframer-1of5prv">
                  <div className="divframer-fxisxp">
                    <div className="link-home">Home</div>
                  </div>
                  <div className="divframer-fxisxp">
                    <div className="link-team">Team</div>
                  </div>
                  <div className="divframer-of6qw9">
                    <div className="link-job">Job</div>
                  </div>
                  <div className="divframer-fxisxp">
                    <div className="link-docs">Docs</div>
                  </div>
                  <div className="divframer-fxisxp">
                    <div className="link-blog">Blog</div>
                  </div>
                </div>
              </div>
              <div className="divframer-19pog0s">
                <div className="divframer-136mr39">
                  <div className="heading-6">Apps</div>
                </div>
                <div className="divframer-15ou3">
                  <div className="divframer-fxisxp">
                    <div className="link-phi">Experiences</div>
                  </div>
                  <div className="divframer-fxisxp">
                    <div className="link-phi1">Level</div>
                  </div>
                  <div className="divframer-fxisxp">
                    <div className="link-playground">Others</div>
                  </div>
                </div>
              </div>
              <div className="divframer-139hzgd">
                <div className="divframer-398hum">
                  <div className="heading-6">Resources</div>
                </div>
                <div className="divframer-1of5prv">
                  <div className="divframer-fxisxp">
                    <div className="link-partnership">Partnership</div>
                  </div>
                  <div className="divframer-fxisxp">
                    <div className="link-brand">Brand guide</div>
                  </div>
                </div>
              </div>
              <div className="divframer-139hzgd">
                <div className="divframer-1stdbbb">
                  <div className="heading-6">{`Trust & Legal`}</div>
                </div>
                <div className="divframer-y2d4sq">
                  <div className="divframer-fxisxp">
                    <div className="link-privacy">Privacy policy</div>
                  </div>
                  <div className="divframer-f4hjqm">
                    <div className="link-terms">Terms of service</div>
                  </div>
                </div>
              </div>
            </div>
            <img className="divframer-19w2hbr-child" alt="" src="/ellipse-12@2x.png" />
          </div>
          <div className="divframer-1nvcxc">
            <div className="divframer-1fkbef0">
              <div className="divframer-tvlwu1">
                <div className="copyright-2022">Copyright © 2022 L3arn.</div>
              </div>
              <div className="divframer-575ka8">
                <div className="divframer-g8h6s6-container">
                  <div className="img">
                    <div className="frame120">
                      <img className="vector-icon123" alt="" src="/vector45.svg" />
                    </div>
                  </div>
                </div>
                <div className="divframer-g8h6s6-container">
                  <div className="img1">
                    <div className="frame121">
                      <img className="vector-icon124" alt="" src="/vector46.svg" />
                    </div>
                  </div>
                </div>
                <div className="divframer-g8h6s6-container">
                  <div className="img">
                    <div className="frame122">
                      <img className="vector-icon125" alt="" src="/vector47.svg" />
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default HomePage;
